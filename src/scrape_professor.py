...
from .exceptions import NetworkException, ParsingException, ScrapeException
...
async def scrape_professor(session: aiohttp.ClientSession, url: str) -> Professor:
    try:
        async with session.get(url) as response:
            ...
            for review_element in soup.select(".Rating__StyledRating-sc-1rhvpxz-1"):
                votes_list = review_element.select(
                    ".Thumbs__HelpTotalNumber-sc-19shlav-2"
                )
                helpfulVotes = ""
                unhelpfulVotes = ""
                ...
                review = ProfessorReview(
                    ...
                    tags=[
                        tag.text.strip()
                        for tag in review_element.select(".Tag-bs9vf4-0")
                    ],
                )
        ...
        raise ParsingException(f"Parsing error occurred: {e}")
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        raise ScrapeException(f"Unexpected error occurred: {e}")
